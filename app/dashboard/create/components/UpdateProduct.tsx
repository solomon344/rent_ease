...
const UpdateProperty = ({amenities,property}: {amenities: any[],property:any}) => {
  const [formData, setFormData] = React.useState({
    ...
    selectedAmenities: property?.amenities?.map((a:any)=>String(a.description).toLowerCase()),
    // @ts-ignoreA
    tags:property.tags,
  })
//   console.log(property)
  const [imageFile, setImageFile] = React.useState<File | null>(null)
  const [imagePreview, setImagePreview] = React.useState<string>(property.imageUrl)
  ...
  const handleAmenityChange = (amenityKey: string, checked: boolean) => {
    setFormData(prev => ({
      ...
      selectedAmenities: checked
        ? [...prev.selectedAmenities, amenityKey.toLowerCase()]
        : prev.selectedAmenities.filter((key:any) => key.toLowerCase() !== amenityKey.toLowerCase())
    }))
  }
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    ...
    const amenities_list = formData.selectedAmenities.map((amenityKey:string) => {
      return amenities.find((amenity) => String(amenity.name).toLowerCase() === amenityKey.toLowerCase())?.id
    })
    
    try{
      if (imageFile){
        const result = await edgestore.rentEaseImages.upload({file:imageFile as File, options:{replaceTargetUrl:imagePreview}});
        setImagePreview(result.url)
      }
      const response = await Api.post(`/properties/update/${property.id}/`, {
        name:formData.name,
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        size='lg'
                        radius='sm'
                        defaultSelectedKeys={[String(formData.location).toLowerCase()||"",]}
                        className="w-full focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        value={String(formData.location).toLowerCase()||""}
                ...
                <CardBody>
                  <div className="grid grid-cols-1 gap-3">
                    {amenities.map((amenity) => (
                      <Checkbox
                        key={amenity?.name}
                        isSelected={formData.selectedAmenities.includes(String(amenity.name).toLowerCase())}
            ...
            <div className="lg:col-span-2">
              <Card className="bg-white shadow-sm sticky top-6">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-600 ml-3">See how your property will appear to guests</p>
                </CardHeader>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {formData.selectedAmenities.length > 0 ? (
                            formData.selectedAmenities.slice(0, 6).map((amenityKey:string) => {
                              const amenity = amenities.find(a=>String(a.name).toLowerCase() === amenityKey)
                              return (
